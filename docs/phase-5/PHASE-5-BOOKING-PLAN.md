...
|  - Immutable JSONB historical snapshots (Package, Itinerary, Price)         |
|  - Passenger validation & structured roster storage                         |
|  - Booking state transitions (PENDING, CONFIRMED, CANCELLED, EXPIRED)       |
...
|  - Automated payment provider refund execution                              |
...
- **Answer:** `booked_seats` increases **ONLY** when a booking transitions to `CONFIRMED` (e.g. upon payment confirmation in Phase 6 or manual admin confirmation).
- During checkout in Phase 5, `booked_seats` is NOT incremented; seats are protected strictly via the active `inventory_holds` entry. When confirmed, `booked_seats` is atomically incremented, and the hold status is transitioned to `COMMITTED`.
...
- **Answer:** Held seats are released under three conditions:
  ...
  2. **Expiration:** 15 minutes elapse without confirmation (`expires_at <= NOW()`), after which PostgreSQL availability queries automatically treat the hold as inactive, and the background worker updates status to `'EXPIRED'`.
...
- **Answer:** The payment webhook retrieves the booking, executes a transaction locking the departure, verifies that the hold is still `ACTIVE` (or that sufficient seats still exist), atomically increments `booked_seats = booked_seats + party_size`, marks the hold as `COMMITTED`, and transitions the booking status to `CONFIRMED`.
...
- **Answer:** The client resends the request with the same `Idempotency-Key` header. The backend detects the existing completed transaction for that idempotency key and returns the identical cached response (including booking reference and hold details) without re-executing seat reservation or creating duplicate holds.
...
[Incoming Request: POST /api/v1/bookings with Idempotency-Key]
   │
   ├─► Check Idempotency Cache (Redis / PostgreSQL)
   │     └─ If matched: Return cached response immediately
   │
   ...
   │     │     INSERT INTO bookings (..., package_snapshot, price_breakdown, status='AWAITING_PAYMENT')
   │     │
   │     ├─ 6. Insert Passenger Roster:
   │     │     INSERT INTO booking_passengers (booking_id, full_name, age, gender, passenger_type)
   │     │
...
3. **Replay & Concurrency Protection:**
   - Enforce mandatory `Idempotency-Key` validation on all state-mutating endpoints (`POST /bookings`, `POST /bookings/:ref/cancel`).

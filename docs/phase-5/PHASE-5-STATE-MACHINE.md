...
```
  ...
  booked_seats += partySize  │                   │
             ...
             │   (Confirmed Booking) │   │  (Hold Dead)  │ │ (Void Booking)│
             │                       │   │               │ │               │
             └───────────┬───────────┘   └───────────────┘ └───────────────┘
                         │
...
| **`CONFIRMED`**        | Non-Terminal | Payment verified or Admin manual override | Seats permanently committed to `booked_seats`                                                | Paid in full                     | Visible as "Confirmed"; manifest eligible         | Visible on departure manifest & booking ledger |
| **`CANCELLED`**        | Terminal     | Customer/Admin cancel action executed     | If hold active $\rightarrow$ Released; if confirmed $\rightarrow$ `booked_seats` decremented | Refund policy applied in Phase 6 | Visible as "Cancelled"                            | Visible in audit trail & cancellations ledger  |
...
| `AWAITING_PAYMENT` | `PAYMENT_CAPTURED` _(Phase 6)_ | `CONFIRMED`        | - Hold must be `ACTIVE` and `expires_at > NOW()`<br>- Payment amount matches total | `booked_seats += partySize`<br>Hold status $\rightarrow$ `COMMITTED` |
...
| `CONFIRMED`        | `CUSTOMER_CANCEL_BOOKING`      | `CANCELLED`        | - Cancellation allowed if departure is in the future                               | `booked_seats -= partySize`<br>Seats returned to public availability |
| `CONFIRMED`        | `ADMIN_CANCEL_BOOKING`         | `CANCELLED`        | - Admin reason provided for audit log                                              | `booked_seats -= partySize`<br>Seats returned to public availability |

---

## 4. Inventory Hold Lifecycle State Machine

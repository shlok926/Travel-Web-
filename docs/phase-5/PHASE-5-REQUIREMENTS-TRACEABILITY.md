...
| **FR-BOOK-002**   | Structured passenger schema validation    | Validate name, age, gender, passenger type, and contact details            | `booking_passengers`                                                 | `POST /api/v1/bookings`                              | `TEST-BOOK-002`   | [DOCUMENTED] |
| **FR-BOOK-003**   | Immutable historical snapshots            | Modifying catalogue package/price/itinerary never alters past bookings     | `bookings.package_snapshot`, `price_breakdown`, `itinerary_snapshot` | `POST /api/v1/bookings`, `GET /api/v1/bookings/:ref` | `TEST-BOOK-003`   | [DOCUMENTED] |
...
| **FR-BOOK-005**   | Hold expiry & automatic release           | Unpaid holds expire after 15m; availability automatically restored         | `inventory_holds.status = 'EXPIRED'`, BullMQ Queue                   | BullMQ Background Sweeper                            | `TEST-BOOK-005`   | [DOCUMENTED] |
...
| **FR-DASH-004**   | Self-service booking cancellation         | Cancel unconfirmed or eligible bookings; release held or booked seats      | `bookings.status = 'CANCELLED'`                                      | `POST /api/v1/bookings/:ref/cancel`                  | `TEST-DASH-004`   | [DOCUMENTED] |
...
| **NFR-SEC-004**   | Idempotency on payment & booking retries  | Unique `Idempotency-Key` prevents duplicate bookings on network retries    | `idempotency_keys`                                                   | `POST /api/v1/bookings`                              | `TEST-SEC-004`    | [DOCUMENTED] |

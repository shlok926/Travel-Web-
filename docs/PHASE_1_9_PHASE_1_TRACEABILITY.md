...
| **`ADR-002`** (Vanilla Web SPA)         | Modular Frontend Bootstrap & ApiClient | `frontend/src/main.js`, `frontend/src/api/client.js`             | Browser inspection & prototype check                 | **PASS** |
...
| **`ADR-004`** (PostgreSQL 15+)          | Database Pool & Transaction Utility    | `backend/src/infrastructure/database/index.ts`                   | `tests/resilience/failure.test.ts`                   | **PASS** |
| **`ADR-005`** (Argon2id + JWT)          | Password & Token Security Modules      | `shared/src/security/argon2.ts`, `shared/src/security/jwt.ts`    | `tests/unit/security.test.ts`                        | **PASS** |
| **`ADR-006`** (RESTful API / Envelopes) | RFC 7807 Error Handler & Envelopes     | `backend/src/plugins/errorHandler.ts`, `shared/src/types/api.ts` | `tests/integration/errors.test.ts`                   | **PASS** |
...
| **`ADR-008`** (Object Storage)          | Pluggable IStorageService Adapters     | `backend/src/infrastructure/storage/`                            | `tests/unit/storage.test.ts`                         | **PASS** |

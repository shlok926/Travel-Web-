...
| `JWT_ALGORITHM`          | String  | `HS256`                 | **YES**             | JWT algorithm (`RS256` standard, `HS256` for dev).                 |
| `JWT_SECRET_KEY`         | String  | `super_secure_...`      | **YES (for HS256)** | Secret key (minimum 32 characters long).                           |
...
| `STORAGE_DRIVER`         | String  | `local`                 | **YES**             | Storage driver (`local` or `s3`).                                  |
| `STORAGE_LOCAL_PATH`     | String  | `./uploads`             | No                  | Local filesystem path for file uploads.                            |
